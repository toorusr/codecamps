...
func fetchCamps() ([]Event, error) {
	...
	doc.Find("section.box ul li").Not(".pdf").Each(func(i int, s *goquery.Selection) {
		camps = append(camps, Event{Datum: strings.Replace(strings.SplitAfter(s.Text(), ",")[0], ",", "", -1), Ort: strings.Replace(strings.SplitAfter(s.Text(), ",")[1], ",", "", -1)})
	})
	return camps, nil
}
...
func fetchWorkshops() ([]Event, error) {
	...
	doc.Find("section.box ul li").Each(func(i int, s *goquery.Selection) {
		workshops = append(workshops, Event{Datum: strings.Replace(strings.SplitAfter(s.Text(), ",")[0], ",", "", -1), Ort: strings.Replace(strings.SplitAfter(s.Text(), ",")[1], ",", "", -1)})
	})
